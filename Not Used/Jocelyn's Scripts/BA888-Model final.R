...
best_fw_model <- as.formula("is_open ~ review_count + full_bar + park_lot + stars + 
                            park_street + attributes_WiFi + amb_trendy + beer_wine +
                            attributes_BikeParking + amb_romantic + attributes_RestaurantsReservations +
                            oct + dec + mar + aug + amb_hipster + park_valet + park_validated +
                            park_garage + amb_touristy + nov + amb_casual")
...
# From the above results, it is clear that park_street, park_validated,amb_romantic, full_bar, attributes_BikeParking, park_lot, stars, and amb_trendy have higher correlations to whether the restaurant is open or not.
# I would say that customers pay more attention to parking in restaurants and restaurants stars. Therefore, the restaurant's open and close have a strong relationship with parking and stars.
# The accuracy of forward stepwise regression is almost 72%.
...
confusionMatrix(data_test$is_open, predicted, threshold = optCutOff)
### I want to remove review_count variable, although I tried winsorized but it is still an outliner in the model.

# The final random forest model included 30 variables. 
...
# On the plot we can see "sep", "aug", "jul", "apr", "jun", "may", "oct", "feb", "mar", "jan", "nov", "dec", and "star" are the most predictive with
# IncNodepurity all higher than 45. We got the accurcy rate for this model is almost 72%.